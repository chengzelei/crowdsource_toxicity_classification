...
class SLDROTrainer(Trainer):
    ...
    def _set_signature_columns_if_needed(self):
        if self._signature_columns is None:
            ...
            self._signature_columns = list(signature.parameters.keys())
            # Labels may be named label or label_ids, the default data collator handles that.
            self._signature_columns += list(set(["dominant_topics", "label", "label_ids"] + self.label_names))
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if "dominant_topics" in inputs:
            dominant_topics = inputs.pop("dominant_topics")
        outputs = model(**inputs)
        ...
        if labels is not None:
            ...
            unique_topics = dominant_topics.unique()
            topic_means = torch.stack([loss[dominant_topics == topic].float().mean() for topic in unique_topics])
            loss = topic_means.max()
